import { getDramaMovieService } from '@/services/movie.service'
import React from 'react'

const DramaMovieComponent = async () => {
    const movieData = await getDramaMovieService();
    console.log("drama movie data",movieData)
        ...
        {movieData.payload.map((data) => (
            <div>
                ...
                <a class="p-8 max-w-lg border border-gray-200 rounded-2xl hover:shadow-xl hover:shadow-indigo-50 flex flex-col items-center"
                   ...
                   <img src={data.image == ''? '../bg-img.png' : data.image }  class="shadow rounded-lg overflow-hidden border" />
                    <div class="mt-8 ">
